...
- `$ae-enrich`（自动选 effective backlink 最多的红链；普通 markdown mention 不会抬高优先级）
...
ingest Stage 4 仍会对强证据 company 按 wikilink 的 dir 前缀建 page——agent 把 `[[companies/Trainium]]` 拼错成 `companies/` 前缀，仍可能建出一个 type 错误的 stub。concept / industry 已改为 candidate-first。在投真正 enrich narrative 之前**先判断 type 对不对**：
