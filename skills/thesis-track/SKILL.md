---
name: thesis-track
description: 投资论点状态机维护——开仓 / 写 Bull-Bear / 更新 conviction / 标 catalyst 命中 / 关仓归档。每次 ingest 后自动检查 active thesis 是否有新证据；新建论点时帮 PM 结构化思考（验证 / 证伪条件 + 催化剂日历）。core 不调 LLM，agent 当 PM 助手。
metadata:
...
2. 自动跟踪：每次 ingest 新 source，看是否有命中验证条件的 fact 或反例
3. 周期性 review，按证据动态调整 conviction
4. 触发关仓：validated / invalidated / stop_loss

`$thesis-track` 是这个闭环的入口。

## 何时调用

| 场景 | 推荐操作 |
|---|---|
| PM 路演 / 用户说"我看好 X / 看空 Y" | `$thesis-track open` 帮他结构化 |
| 跑完 `$research-ingest` 触及某 active thesis 的 target | 自动看有没有新 signal，触发 review |
| 用户问"我 NVDA 的论点最近怎么样" | `$thesis-track show <id>` 出诊断 |
| 月度 review / 早会 | `$thesis-track list --status active` 全清单 |
| 关仓决策 | `$thesis-track close --reason validated/invalidated/stop_loss` |
...
[agent]
  ...
  4. 按论点模板写 narrative（参考 templates/thesis-template.md）：
...
### B. ingest 新 source 后自动 review

每次跑完 `$research-ingest`，agent 应该：

```
1. bun src/cli.ts thesis:list --status active --status monitoring
   → 找出所有可能受影响的 thesis
...
3. 如果有重大变化，告诉用户："论点 X 需要你 review"
```
...
research-ingest          ──→ 写 facts / signals
   ↓
   每次 ingest 完成     →  自动检查相关 active thesis（这个 skill）
                       →  调 thesis:update mark-condition
...
| 症状 | 原因 | 解决 |
|---|---|---|
| `thesis:open` 报 target page 不存在 | target 还没 enrich 或 slug 拼错 | 先 `$enrich` 或核对 slug |
