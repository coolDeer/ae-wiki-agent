...
> 给 Codex 看的项目说明。包含**业务层**（投资 wiki schema / 工作流 / 约定）+ **代码层**（工程约定 / 命令 / 坑点）。
...
面向投资研究的知识库 + agent 工作台。把 mongo / 平台 API 来的研报、电话会、调研纪要等素材，转成结构化、可查询、可推理的投资研究底座。Codex 是主要编辑者；PM / 分析师提供素材、提问、做决策。
...
# 后台
bun src/cli.ts jobs:worker                      # minion_jobs 队列处理
...
需要 LLM 推理的主流程由 agent runtime 执行（如 ingest / enrich / daily-*）；skill 协议在 `./skills/ae-*/SKILL.md` 里，可热改。
...
老的 llm-wiki/ 父目录现在只剩 `wiki/`（只读历史）+ `templates/` + `karpathy LLM Wiki.md` + 根 AGENTS.md/AGENTS.md（与本文件高度重叠，可删）。
