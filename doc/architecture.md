...
┌──────────────────▼───────────────────────────────────────────────┐
│                       Layer 3: Search                              │
│   keyword (tsvector + 中文分词)  │  vector (pgvector HNSW)         │
...
> **中文分词说明**：`'simple'` 配置不分中文，需要在 ingest 前应用层用 jieba 分词后再写入；或装 zhparser 扩展。MVP 用 simple + jieba 预分词。
...
任何对一个 page 的 sidecar 元数据（mineru content_list / 第三方 API 拉取等）。

```sql
CREATE TABLE raw_data (
  id          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  page_id     BIGINT NOT NULL,
  source      TEXT NOT NULL,                      -- 'mineru_content_list' | 'aecapllc_api' | 'finance_api' | ...
  data        JSONB NOT NULL,
