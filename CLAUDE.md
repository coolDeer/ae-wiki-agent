...
正文语言：**中文为主**，专业术语保留英文（P/E、EBITDA、TAM、ROIC、SAF、HBM 等）。
...
│   │   ├── tokenize.ts       # jieba 切词（cutForSearch / cut 双模式）
│   │   ├── types.ts
│   │   ├── mongo.ts
│   │   ├── schema/           # Drizzle schema（与 infra/init-v2.sql 同步）
│   │   └── search/
│   │       ├── hybrid.ts     # RRF + source-boost + jieba
│   │       └── source-boost.ts
│   ├── skills/
│   │   ├── fetch-reports/
...
├── scripts/
│   ├── deploy-schema.ts
│   ├── run-jieba-migration.mjs
│   ├── smoke-test-jieba.mjs
│   └── cron/
...
|---|---|---|
| `sources` | 多租户分区 | id |
| **`pages`** | **万物皆 page** | slug / type / content / tokens_zh / aliases / ticker / sector / confidence |
...
| `minion_jobs` | 异步队列 | name, status, data, attempts |
| `config` | KV | id, value |
...
写 narrative 时按 page type 选模板，落地到 `pages.content`。frontmatter 里的核心字段（ticker/sector 等）通过 CLI flag 写入 pages 表对应列；其他元数据进 `pages.frontmatter` JSONB。

### 1. Source（来源摘要）

正文必含 7 段 + 末尾 YAML 块（供 Stage 5/7 直读）。详见 `./skills/research-ingest/SKILL.md`。
...
## 重要数据点         # 表格优先
## 值得注意的观点/引语 # blockquote 保留原文
                    # 优先：管理层表态、专家对结构性问题的判断、反直觉观点
## 结构性观察         # 非数字判断（不得省略，没有则写"无"）
...
## 催化剂时间线
## 风险管理          # stop loss / re-evaluate triggers
## 持仓逻辑演进      # 每次 conviction 变化记录
## 引用来源
```

`catalysts` 和 `validation_conditions` 用 `thesis:update --add-catalyst / --mark-condition` 命令结构化进 JSONB。详见 `./skills/thesis-track/SKILL.md`。
...
`pages.tokens_zh` 列存 jieba 切完的空格分隔串，trigger 用它喂 tsvector。Ingest 写 narrative 时由应用层算（`src/core/tokenize.ts`）。已经过测试中英文混合场景。
...
| `query_facts(entity, metric?, period?)` | 结构化 fact 查询 |
...
| 中文分词 | `@node-rs/jieba` | 见下方"jieba 坑点" |
...
bun install
cp .env.example .env
# 填 DATABASE_URL / MONGODB_URI / OPENAI_API_KEY / ANTHROPIC_API_KEY
...
write          → stdin 落库（page.content + tokens_zh + page_versions 快照）
...
```
title (A) || aliases (A) || COALESCE(tokens_zh, content) (B) || timeline (C)
```

**应用层职责**：写 `pages.content` 时同步算 `tokens_zh = tokenizeForIndex(content)`。trigger 只负责把这些字段拼成 tsvector。

查询时也要 tokenize：`hybridSearch` 把 query 喂 `tokenizeForQuery` 再传 `plainto_tsquery`。
...
`@node-rs/jieba` 是 Rust native binding，bun 加载时**必须从项目 cwd 起跑**：

```bash
# ✅ 在 ae-wiki-agent/ 目录下跑
cd ae-wiki-agent
bun src/cli.ts foo

# ❌ 从别的地方跑会报 "Failed to load native binding"
bun /Users/levin/project/.../ae-wiki-agent/src/cli.ts foo
```

原因：bun 的 package resolution 优先 `~/.bun/install/cache/`（缓存里没 native binding），而不是本地 `node_modules`。从 cwd 跑能让 bun 正确解析到 `node_modules/@node-rs/jieba-darwin-arm64/`。

`scripts/cron/*.sh` 都做了 `cd "$PROJECT_DIR"`，cron 路径下没问题。
...
- keyword 通道：`tokens_zh` 喂 `to_tsvector('simple', ...)` + ts_rank × source_factor
...
唯一需要 LLM 推理的是 ingest Step 2（agent 阅读 raw + 写 narrative），由 Claude Code 这一侧执行——prompt 在 `./skills/research-ingest/SKILL.md` 里，可热改。
...
| 词典 hot-reload 没接 | 新建 entity 后 jieba 不知道 | ⭐ |
