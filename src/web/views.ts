...
export async function viewHome(): Promise<string> {
  ...
  const outputs = await listOutputPages(8);
...
<div class="grid">
  ...
  <div class="card">
    ...
    ${outputs.length === 0
      ? `<div class="empty">no output pages</div>`
      : `<ul class="plain">${outputs
          .map(
            (f) =>
              `<li><a href="/outputs/${encodeURIComponent(f.name)}">${escape(f.title)}</a> <span class="muted score">${escape(f.mtime)}</span></li>`
          )
          .join("")}</ul>
...
async function listOutputPages(limit = 100): Promise<OutputPageSummary[]> {
  const rows = await db
    .select({
      slug: schema.pages.slug,
      title: schema.pages.title,
    ...
    .from(schema.pages)
    .where(and(eq(schema.pages.deleted, 0), eq(schema.pages.type, "output")))
    .orderBy(desc(schema.pages.updateTime))
    .limit(limit);

  return rows.map((row) => ({
    slug: row.slug,
    name: outputSlugName(row.slug),
    title: row.title,
    mtime: fmtSh(row.updateTime),
    charCount: (row.content ?? "").length,
  }));
}

export async function viewOutputs(): Promise<string> {
  const files = await listOutputPages();
  const body = `
<h2>Outputs (${files.length})</h2>
<p class="muted">Generated by <code>$ae-daily-review</code> / <code>$ae-daily-summarize</code> as output pages.</p>
${files.length === 0
  ? `<div class="empty">no outputs</div>`
  : `<ul class="plain">${files
      .map(
        (f) =>
          `<li><div class="row"><div class="grow"><a href="/outputs/${encodeURIComponent(f.name)}">${escape(f.title)}</a> <span class="muted score">${escape(f.slug)}</span></div><span class="muted score">${escape(f.mtime)} · ${f.charCount} chars</span></div></li>`
      )
      .join("")}</ul>`
}
`;
...
export async function viewOutputFile(filename: string): Promise<string> {
  ...
  const slug = filename.startsWith("outputs/")
    ? filename
    : `outputs/${filename.replace(/\.md$/, "")}`;
  const [page] = await db
    .select({
      slug: schema.pages.slug,
      title: schema.pages.title,
    ...
    .from(schema.pages)
    .where(and(eq(schema.pages.deleted, 0), eq(schema.pages.type, "output"), eq(schema.pages.slug, slug)))
    .limit(1);
