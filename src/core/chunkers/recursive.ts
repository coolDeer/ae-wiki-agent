 ...
 *
 * Borrowed and adapted from `demo/gbrain/src/core/chunkers/recursive.ts`.
 * Goal: keep chunking deterministic, loss-aware, and substantially better
 * than the current paragraph-only fallback.
 */
...
function applyOverlap(chunks: string[], overlapWords: number): string[] {
  ...
  for (let i = 1; i < chunks.length; i++) {
    ...
    result.push(extractTrailingContext(prev, overlapWords) + current);
  }
